...
def contact(request):    
    if request.method=='POST':
        ...
        if len(name)<2 or len(email)<3 or len(phone)<10 or len(content)<4:
            ...
            contect = Contact(name=name, email=email,phone=phone,content=content)
            contect.save()
            messages.success(request,'Your message has been sent succssfully!')
...
def handleSignup(request):
    if request.method == 'POST':
        ...
        # username sholud be alpha numeric
